...
#		logger.error(argerror.args)
	...
	try:
		...
		with hide('output','running','warnings'),settings(warn_only=True):
			...
			if results[t.host] == 1:
				if t.dr == True:
					# this print is not needed, remove it when you want
					print("Dryrun success!")
				else: 
					# this print is not needed, update orderbox with admin password instead
					print("admin pass : %s"%t.adminp)
					# Call Orderbox here with adminpass, and orderID
			if results[t.host] == 2:
